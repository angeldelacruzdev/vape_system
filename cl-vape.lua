	...
	if DoesEntityExist(ped) and not IsEntityDead(ped) then
		if IsPedOnFoot(ped) then
			if IsPlayerAbleToVape == false then
				...
				ShowNotification("~c~Has ~b~comenzado ~c~a usar tu vape.")
				Wait(Config.HelpTextLength)
				TriggerEvent("Vape:HelpFadeOut", 0)
			else
				ShowNotification("~r~Ya estás sosteniendo tu vape.")
			end
		else
...
AddEventHandler("Vape:StopVaping", function(source)
	if IsPlayerAbleToVape == true then
		...
		ShowNotification("~c~Dejaste ~r~de usar ~c~tu vape.")
	else
		ShowNotification("~r~No estás sosteniendo tu vape.")
	end
end)
AddEventHandler("Vape:Drag", function()
	if IsPlayerAbleToVape then
		...
		ShowNotification("~r~Debes sostener tu vape para hacer esto.")
	end
end)
	...
	while true do
		if IsPlayerAbleToVape then
			if DisplayText then
				local ped = GetPlayerPed(-1)
				local pedPos = GetEntityCoords(ped)
				DrawText3d(pedPos.x, pedPos.y, pedPos.z+1.20, "~c~Presiona ~b~\"E\"~c~ Para usar el vape.")
				DrawText3d(pedPos.x, pedPos.y, pedPos.z+1.08, "~c~Presiona ~b~\"G\"~c~ para restablecer el vape.")
			end
		end
	  Wait(1)
	end
